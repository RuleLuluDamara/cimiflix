...
<div class="col-lg-8">
    <form method="post" action="/dashboard/movies/{{ $movie->slug }}">
      @method('put')
      @csrf
      <div class="mb-3">
        <label for="name" class="form-label">Title</label>
        <input type="text" class="form-control @error('name') is-invalid
        @enderror" id="name" name="name" required autofocus value="{{  old('name', $movie->name ) }}">
        @error('name')
      ...
      <div class="mb-3">
        ...
        <select class="form-select"  name="rating_umur_id" >
          ...
          @if (old('rating_umur_id', $movie->rating_umur_id) === $rating_umur->id)
      ...
      <div class="mb-3">
        ...
        <select class="form-select"  name="status_id" >
          ...
          @if (old('status_id', $movie->status_id) === $status->id)
      ...
      <div class="mb-3">
        ...
        @enderror" id="rilis" name="rilis" value="{{  old('rilis', $movie->rilis) }}">
      </div>
      <div class="mb-3">
        <label for="resolusi" class="form-label">Resolusi</label>
        <input type="text" class="form-control @error('resolusi') is-invalid
        @enderror" id="resolusi" name="resolusi" value="{{  old('resolusi', $movie->resolusi) }}">
      </div>
      <div class="mb-3">
        <label for="durasi" class="form-label">Durasi</label>
        <input type="text" class="form-control @error('durasi') is-invalid
        @enderror" id="durasi" name="durasi" value="{{  old('durasi', $movie->durasi) }}">
      </div>
      <div class="mb-3">
        <label for="director" class="form-label">Director</label>
        <input type="text" class="form-control @error('director') is-invalid
        @enderror" id="director" name="director" value="{{  old('director', $movie->director) }}">
      </div>
      <div class="mb-3">
        ...
        @enderror" id="studio_production" name="studio_production" value="{{  old('studio_production', $movie->studio_production) }}">
      </div>
      <div class="mb-3">
